...
def read_test_data():   # 读取测试数据
    file_dir = 'C:/Users/vike0906/Documents/WeChat Files/liujialonglove' \
               '/FileStorage/File/2020-04/wiki_w_dd_structure.txt'
    source_data = []
    with open(file=file_dir, mode='r', encoding='utf-8') as source_file:
        for line in source_file:
            line = '\''+line.replace('\t', '\', \'', 4).replace('\t', '\', ', 1)
            line = line.replace('\t', ',', 2).replace('\t', ', \'', 1).replace('\t', '\', \'', 3)+'\''
            line = line.replace('[]', '')
...
if __name__ == '__main__':
    ...
    while len(data) > 0:  # 拼接批量插入sql
        insert_sql = 'insert into w_dd_structure values '
        if len(data) > 100:
            for i in range(100):
